...
a =1""")
        ...
        context = self._makeOne(inifile, buildoutdir='/tmp', logger=logger)
        self.assertEqual(context.inifile, inifile)
        self.assertEqual(len(context.globals), 6)
        self.assertEqual(context.globals['buildoutdir'], '/tmp')
...
b = 1""")
    ...
    def test_Context_resolve_default_namespace(self):
        inifile = self._makeFile('root.ini',
            """\
[namespaces]
pound = %s/pound.ini
...
b = 1""")
    ...
    def test_Context_resolve_nodefault_namespace(self):
        inifile = self._makeFile('root.ini',
            """\
[namespaces]
pound = %s/pound.ini
...
class TestSkelCopier(unittest.TestCase):
    ...
    def _makeFilePath(self, to, text):
        ...
        try:
            ...
            pass
        filename = os.path.join(fulldir, path)
        f = open(filename, 'w')
        f.write(text)
        return to

    def test_copier(self):
        infile1 = self._makeFilePath('foo.in', 'Foo is <<FOO>>')
        infile2 = self._makeFilePath('in/in/in/foo.in', 'Foo is <<FOO>>')
        f1 = self._makeFilePath('1.txt', 'This is one')
        ...
        class DummyInterpolator:
            def interpolate(self, thing):
                return thing
            def interpolate_kw(self, **kw):
                return {'skeldir':tempdir, 'tgtdir':tgtdir}
        interpolator = DummyInterpolator()
        r = copier.represent(interpolator)
        c = copier.check(interpolator)
        self.assertEqual(r, 'SkelCopier from %s to %s' % (tempdir, tgtdir))
        self.assertEqual(c, 'SkelCopier from %s to %s' % (tempdir, tgtdir))
        copier.execute(interpolator)
        self.assertEqual(open(os.path.join(tgtdir, f1)).read(), 'This is one')
        self.assertEqual(open(os.path.join(tgtdir, f2)).read(), 'This is two')
        self.assertEqual(open(os.path.join(tgtdir, f3)).read(), 'This is three')
        self.assertEqual(open(os.path.join(tgtdir, f4)).read(), 'This is four')

        self.assertEqual(open(os.path.join(tgtdir, 'foo')).read(),
                         'Foo is ${./foo}')
        self.failIf(os.path.exists(os.path.join(tgtdir, 'foo.in')))
        self.assertEqual(open(os.path.join(tgtdir, 'in/in/in/foo')).read(),
                         'Foo is ${./foo}')
        self.failIf(os.path.exists(os.path.join(tgtdir, 'in/in/in/foo.in')))
